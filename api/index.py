...
# Create the Flask app - point to root folders for templates and static
app = Flask(__name__, 
...
# Note: Vercel has a read-only filesystem. Use an external DB (like PostgreSQL) for persistence.
...
with app.app_context():
    try:
        ...
        app.logger.warning(f"Could not create DB tables (expected on read-only Vercel with SQLite): {str(e)}")
